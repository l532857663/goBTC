...
func (builder *InscriptionBuilder) UpdatePSBT(updater *psbt.Updater) error {
	for i, in := range builder.CommitTxPrevOutputList {
		prevPkScript, err := AddrToPkScript(in.Address, builder.Network)
		if err != nil {
			return err
		}
		if txscript.IsPayToPubKeyHash(prevPkScript) {
			fmt.Printf("wch----- NonWitnessUTXO\n")
...
func (builder *InscriptionBuilder) UpdateCommitInfo(psbtHex string) {
	txHex, err := ExtractTxFromSignedPSBT(psbtHex)
	if err != nil {
		fmt.Printf("ExtractTxFromSignedPSBT error: %+v\n", err)
		return
	}
	...
	builder.CommitTx.Deserialize(bytes.NewReader(txBytes))
	fmt.Printf("new hash: %+v\n", builder.CommitTx.TxHash().String())
}
