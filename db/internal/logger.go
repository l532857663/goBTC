...
func Init(logZapConf string, z *zap.Logger) {
	logZap = logZapConf
	if zapLogger == nil {
		zapLogger = z
	}
...
func (c *customLogger) Printf(message string, data ...interface{}) {
	if logZap != "" {
		...
		case 1:
			zapLogger.Info("gorm", zap.Any("src", data[0]))
		case 2:
			zapLogger.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]))
		case 3:
			zapLogger.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]))
		case 4:
			zapLogger.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]), zap.Any("sql", data[3]))
		}
		return
	}
	switch len(data) {
